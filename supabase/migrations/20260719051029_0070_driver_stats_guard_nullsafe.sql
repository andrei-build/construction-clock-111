-- null-safe owner-гейт: app.is_owner() возвращает NULL без JWT (role in (...) от NULL-роли),
-- «if not NULL» не райзит. Для юзеров PostgREST поведение не меняется.
...
declare
  ...
  v_result jsonb;
begin
  if coalesce(app.is_owner(), false) = false then
