-- SET-1 GPS: экспорт GPS-данных работника (юридика/прозрачность) + удаление трекинг-фидов.
-- Экспорт: owner-only, единый список точек из трёх источников. time_events НЕ удаляем никогда (ядро №2) — только экспорт.
...
revoke all on function app.report_gps_export(timestamptz, timestamptz, uuid) from public;
grant execute on function app.report_gps_export(timestamptz, timestamptz, uuid) to authenticated;

-- Удаление трекинг-фидов (live_locations + shift_geo_events) старше даты / по работнику. Отметки времени НЕ трогаются.
...
revoke all on function app.purge_gps_feeds(timestamptz, uuid) from public;
grant execute on function app.purge_gps_feeds(timestamptz, uuid) to authenticated;
